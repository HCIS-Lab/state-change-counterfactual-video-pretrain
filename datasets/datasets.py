...
class PCA_EGOPROCEL_FRAMES(data.Dataset):
    def __init__(self,
                 small_test=False,
                 transform=None,
                 sliding_window=16):
        ...
        self.data_lst = np.load(
            "/nfs/wattrel/data/md0/datasets/EgoProceL/videos/pc_disassembly_metadata.npy")

    def __getitem__(self, index):
        videoname = self.data_lst[index][0]
        vroot = videoname
        chunk_id = self.data_lst[index][1]
        ...
        start_idx = int(chunk_id) * 128
        end_idx = (int(chunk_id) + 1) * 128
        ...
        if self.sliding_window:
            ...
            # Handle missing frames at boundaries
        ...
        # path_list = [f.decode('utf-8') for f in path_list0]
        # path_list.sort(key=lambda x: int(x[4:-4]))
        # path_list1 = path_list[int(chunk_id) * 128 : (int(chunk_id)+1) * 128]
        # # print(len(path_list))
        # path_list1.sort(key=lambda x: int(x[4:-4]))
        # seq = [Image.open(os.path.join(vroot, p)).convert('RGB') for p in path_list1]
        # if self.sliding_window:
        #     first_frame = seq[0]
        #     last_frame = seq[-1]
        ...
        if self.transform is not None:
            ...
            seq = [convert_tensor(img) for img in seq]
            seq = torch.stack(seq)
        fname = vroot[:-7] + f'_{chunk_id}_cf.npy'
