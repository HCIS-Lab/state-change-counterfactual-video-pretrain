...
class Multi_Trainer_dist_Charades(Multi_BaseTrainer_dist):
    ...
    def __init__(self, args, model, loss, metrics, optimizer, config, data_loader,
                 ...
                 visualizer=None, tokenizer=None, embedder=None, max_samples_per_epoch=50000, start_epoch=1):
    ...
    def tokenize(self, input_str):
        ...
        tokens = self.tokenizer(input_str, padding=True, return_tensors="pt")
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                if self.tokenizer is not None:
                    with torch.no_grad():
                        ...
                        text_embeds = self.embed(data['text'].to(self.device))
                        # print(data['text'].shape)
                        text_embeds = self.allgather(text_embeds[:, 0, :].contiguous(), self.n_gpu, self.args)
                # data['text'] = {key: val.to(self.device) for key, val in data['text'].items()}
                data['video'] = data['video'].to(self.device)

                self.optimizer.zero_grad()
                with torch.set_grad_enabled(True):
                    video_embeds, _ = self.model(data['video'])
                    video_embeds = self.allgather(video_embeds.contiguous(), self.n_gpu, self.args)
    ...
    def _valid_epoch(self, epoch):
        ...
        cls_arr = []
        with open('/nfs/charjabug/data/md1/datasets/charades-ego/CharadesEgo/Charades_v1_classes.txt',
                  'r') as charades:
        ...
        with torch.no_grad():
            ...
            tkns = self.tokenize(cls_arr)
            text_embed = self.embed(tkns.to(self.device))
            text_embeds = text_embed[:, 0, :].cpu().detach()
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for batch_idx, data in enumerate(tqdm(dl)):
                    ...
                    vid_embed, _ = self.model.module(data['video'], return_embeds=True)
