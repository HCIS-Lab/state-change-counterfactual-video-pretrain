...
class InfoNCE(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds, v_embeds, n_embeds, frame_embeds):
        ...
        mask_bool = mask_bool[:frame_embeds.shape[0], :frame_embeds.shape[0]] #torch.eye(frame_embeds.shape[0]).cuda()
        ...
        tcn_1 = - ( torch.log( torch.sum( ( (torch.exp(sim_0_1/self.temperature) + epsilon) / denom_1 ) * mask_bool, dim=-1 ) ) + \
                    torch.log( torch.sum( ( (torch.exp(sim_1_before/self.temperature) + epsilon) / denom_1 ) * mask_bool, dim=-1 ) )) / 2.0

        tcn_2 = - ( torch.log( torch.sum( ( (torch.exp(sim_3_2/self.temperature) + epsilon) / denom_2 ) * mask_bool, dim=-1 ) ) + \
    ...
    def forward_summary(self, summary_embeds, video_embeds, cf_parent, v_embeds, n_embeds):
        ...
        loss_cf_parent = self.forward_cf(
                        text_embeds=cf_parent, 
                        video_embeds=video_embeds,
                        summ_align=x,
                        mask=mask_bool,
                        )
        loss_cf_parent = alpha*loss_cf_parent
        ...
        loss_dict = {
            ...
            "parent_cf_key": loss_cf_parent.item(),
            # "parent_cf_order": beta*loss_cf_order.item()
        }
        summary_loss = loss_align + loss_cf_parent # + beta*loss_cf_order
