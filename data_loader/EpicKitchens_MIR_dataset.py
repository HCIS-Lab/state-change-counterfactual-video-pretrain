...
class MultiInstanceRetrieval(TextVideoDataset):
    def _load_metadata(self):
        ...
        # Assuming the first column is the video path
        video_col = metadata.columns[2]  # get the name of the first column

        # base_video_dir = self.data_dir  # update this path
        print(self.data_dir)
        print('--------')
        # Construct full path and filter rows
        # def video_exists(folder_name):
        #     participant_id = folder_name.split('_')[0]
        #     # full_path = os.path.join(self.data_dir, participant_id, folder_name+'.MP4')
        #     full_path = os.path.join(self.data_dir, participant_id, 'rgb_frames', folder_name)
        #     is_path = os.path.exists(full_path)
        #     if is_path:
        #         print(full_path)
        #     return is_path

        # metadata_filtered = metadata[metadata[video_col].apply(video_exists)]
    ...
    def _get_video_path(self, sample):
        ...
        # full_video_fp = os.path.join(self.data_dir, participant_id, rel_video_fp+'.MP4')
        # full_video_fp = os.path.join(self.data_dir, participant_id, 'rgb_frames', rel_video_fp)
        full_video_fp = os.path.join(self.data_dir, participant_id, rel_video_fp)
        # print(full_video_fp)
    ...
    def _get_caption(self, idx, sample):
        ...
        if self.split == 'train':
            with open(os.path.join(self.meta_dir, 'relevancy/caption_train/{}.pkl'.format(idx)), 'rb') as f_local:
            ...
            if positive_list != []:
                ...
                if pos < len(self.metadata_sentence) and pos < local_relevancy_mat.shape[0]:
                    return self.metadata_sentence.iloc[pos][1], local_relevancy_mat[pos], pos
            return sample[8], 1, 0
...
if __name__ == "__main__":
    kwargs = dict(
        ...
        data_dir="video_dataset/epic-kitchens/epic-kitchens-rgb-frames",
        meta_dir="video_dataset/epic-kitchens/epic-kitchens-100-annotations-master/retrieval_annotations",
