...
def run():
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm.tqdm(enumerate(data_loader)):
            ...
            # data['text'] = [x.to(device) for x in data['text']]
            # data['text'] = data['text'].to(device)
            text_embed = model.text_module(data['text'])['text_embedding'].cpu().detach()
            ...
            vid_embed_arr.append(vid_embed['video_embedding'].cpu().detach())
            # print(text_embed.shape)
