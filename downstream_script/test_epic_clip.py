...
def run():
    ...
    # model = model.to(device)
    # model.eval()
    # flava_tokenizer = transformers.AutoTokenizer.from_pretrained("facebook/flava-full")
    # flava_model = transformers.FlavaTextModel.from_pretrained("facebook/flava-full").to(device).eval()
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm.tqdm(enumerate(data_loader)):
            ...
            for i in range(data['video'].shape[1]):
                frame = data['video'][0, i]  # shape [3, 224, 224]
                frame = frame.unsqueeze(0)  # shape [1, 3, 224, 224]
                # print(frame.shape)
                with torch.no_grad():
                    emb = clip_model.encode_image(frame)  # shape [1, 512]
                    frame_embeddings.append(emb)

            video_embedding = torch.mean(torch.stack(frame_embeddings), dim=0)  # shape [1, 512]

            # vid_embed = model(data['video'].permute(0,2,1,3,4))
            # vid_embed, _ = model(data['video'], return_embeds=True)
            token_arr = clip.tokenize(data['text']).to(device)
            text_embed = clip_model.encode_text(token_arr).float().cpu().detach()
            # token_arr = tokenize(flava_tokenizer, data['text'])  
            # text_embed = embed(flava_model, token_arr.to(device)).cpu().detach()
            # text_embed, vid_embed = model(data, return_embeds=True)
            # print(text_embed.shape)
            vid_embed_arr.append(video_embedding.cpu().detach())
            # print(text_embed.shape)
    ...
    path_dataframes = '/nfs/wattrel/data/md0/datasets/epickitchen/annotations-1/retrieval_annotations'
    ...
    path_relevancy = "/nfs/wattrel/data/md0/kung/state-aware-video-pretrain/Joint-Part-of-Speech-Embeddings-main/src/scripts/caption_relevancy_EPIC_100_retrieval_test.pkl"
...
if __name__ == '__main__':
    ...
    args.add_argument('-r', '--resume',
                      default='/nfs/wattrel/data/md0/datasets/state_aware/results/EgoClip_CF/models/0226_23_46_03/ckpt_18b_1e5_epoch7_correct.pth',
